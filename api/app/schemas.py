...
class Player(BaseModel):
    ...
    games_played: int = Field(
        ge=10, le=82, alias="G", description="Jogos disputados na temporada"
    )
    games_started: int = Field(
        ge=0, le=82, alias="GS", description="Jogos como titular"
    )
    minutes_per_game: float = Field(
        ge=5, le=60, alias="MP", description="Minutos por jogo"
    )
    three_point_pct: float = Field(
        ge=0, le=1, alias="3P%", description="Percentual de cestas de 3 pontos"
    )
    efg_pct: float = Field(
        ge=0, le=1.5, alias="eFG%", description="Effective field goal percentage"
    )
    ft_pct: float = Field(
        ge=0, le=1, alias="FT%", description="Percentual de lances livres"
    )
    total_rebounds: float = Field(
        ge=0, le=40, alias="TRB", description="Rebotes por jogo"
    )
    assists: float = Field(
        ge=0, le=35, alias="AST", description="Assistências por jogo"
    )
    steals: float = Field(
        ge=0, le=15, alias="STL", description="Roubos de bola por jogo"
    )
    ...
    personal_fouls: float = Field(
        ge=0, le=10, alias="PF", description="Faltas pessoais por jogo"
    )
...
class PredictionResponse(BaseModel):
    ...
    salary_cap_ratio: float = Field(
        ge=0, le=1, description="Fração do teto salarial (0–1)"
    )
    salary_cap: float = Field(
        ge=0, description="Teto salarial do ano utilizado no cálculo"
    )
    season: str = Field(
        min_length=7,
        max_length=7,
